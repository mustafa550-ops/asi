...
describe("AgentCard", () => {
  it("renders agent name and description", () => {
    render(<AgentCard name="Diagnostic" status="ready" description="Hata teşhisi" />);
  ...
  it("uses article role with aria-label", () => {
    ...
    expect(screen.getByRole("article")).toHaveAttribute("aria-label", "Ajan: Intent Judge");
  });

  it("shows last action when provided", () => {
    render(<AgentCard name="Test" status="ready" description="test" lastAction="Analiz tamamlandı" />);
    expect(screen.getByText("Son: Analiz tamamlandı")).toBeInTheDocument();
  });

  it("shows run count when provided", () => {
    render(<AgentCard name="Test" status="ready" description="test" runCount={42} />);
