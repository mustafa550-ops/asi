...
mod tests {
    ...
    #[test]
    fn parse_kline_close_valid() {
        let json = r#"{"k":{"c":"45231.50"}}"#;
        let result = parse_kline_close(json);
        assert_eq!(result, Some(45231.50));
    }

    #[test]
    fn parse_kline_close_missing_field() {
        let json = r#"{"e":"kline"}"#;
        let result = parse_kline_close(json);
    ...
    #[test]
    fn parse_kline_close_invalid_json() {
        let result = parse_kline_close("not json");
        assert_eq!(result, None);
    }

    #[test]
    fn parse_trade_price_valid() {
        let json = r#"{"p":"45231.50"}"#;
        let result = parse_trade_price(json);
        assert_eq!(result, Some(45231.50));
    }

    #[test]
    fn parse_trade_price_missing() {
        let json = r#"{"e":"trade"}"#;
        let result = parse_trade_price(json);
        assert_eq!(result, None);
    }
